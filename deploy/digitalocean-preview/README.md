...
profile, invitation, WorkOS, or source-body rows. It includes only the two M009
identity/path/binding rows named by the attested Preview release manifest.

The Python process binds only to `127.0.0.1`. Caddy is the public ingress and
allows only exact `GET`/`HEAD /jobs`, the two manifest-published detail paths,
and token-protected operator health paths.
...
There is no `/job/*` matcher. `scripts/render_public_job_preview_routes.py`
renders the exact Caddy paths from the same manifest used by the Vercel gateway.
The install script refuses to replace an existing database.

`update.sh` is the bounded update path for this already-installed Preview. It
accepts only staged inputs below `/tmp/wahojobs-preview-stage`, attests the new
projection with the staged release before stopping the service, preserves the
old database/configuration/Caddy files and release target, and restores them
automatically if restart, readiness, or either published detail probe fails.
