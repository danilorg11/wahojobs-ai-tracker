CREATE UNIQUE INDEX uq_product_principals_profile_environment
ON product_principals(principal_id, environment_namespace);
...
CREATE TABLE product_profile_revisions (
  ...
  CHECK (canonical_schema_version = 'canonical_profile_v1'),
  CHECK (length(CAST(structured_profile_json AS BLOB)) BETWEEN 2 AND 131072),
  CHECK (json_valid(structured_profile_json) AND json_type(structured_profile_json) = 'object'),
  CHECK (json_extract(structured_profile_json, '$.schema_version') = canonical_schema_version),
  CHECK (json_extract(structured_profile_json, '$.identity.profile_id') = profile_id),
...
CREATE TABLE product_profile_sources (
  ...
  CHECK (source_type IN ('confirmed_about_you_text', 'user_confirmed_correction')),
  CHECK (
